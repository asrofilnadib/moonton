    ...
    const handleUserSubscribe = id => {
        router.post(
            ...
            }),
            {
                onSuccess: () => {
                    toast.success(
                        "You have successfully subscribed to the subscription plan"
                    );
                },
                onError: () => {
                    toast.error("Failed to subscribe to the subscription plan");
                },
            }
        );
    };
                ...
                <div className="flex justify-center gap-10 mt-[70px]">
                    {subscriptionPlans.map((subscriptionPlan) => (
                        <SubscriptionCard 
                            ...
                            durationInMonths={subscriptionPlan.active_period_in_months}
                            isPremium={subscriptionPlan.name === 'Premium'}
                            onUserSubscribe={() => handleUserSubscribe(subscriptionPlan.id)}
                        />
                    ))}
                </div>
            </div>
